...
List gibbs_multivariate_nuisance_cpp(arma::mat data,         // data
                                     ...
                                     arma::cx_cube f_para_half,   // Square root of parametric spectral density matrix L, Yixuan
  ...
  f_store = lpost_correct_bernsteinGammaWhittle(FZ, f_para_half, psd_current, a_prior, k_theta);
  ...
  for (int i=0; i<Ntotal-1; ++i) {
    ...
    double f_k_star;
    try {
      f_k_star = lpost_correct_bernsteinGammaWhittle(FZ, f_para_half, psd_k_star, a_prior, k_theta);   
