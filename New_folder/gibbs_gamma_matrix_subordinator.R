...
gibbs_m_nuisance <- function(data,
  ...
  if (corrected) {
    ...
    if (toggle) {
      ...
      stopifnot(!is.null(prior_params$nu_Sigma)) # prior parameters for parametric part (Normal Inverse Wishart) - Yixuan
      nu_Sigma <- prior_params$nu_Sigma
      stopifnot(!is.null(prior_params$S_Sigma))  # prior parameters for parametric part (Normal Inverse Wishart) - Yixuan
      S_Sigma <- prior_params$S_Sigma
    } else {
  ...
  if (corrected && toggle) {
    ...
    param__Sigma <- array(NA, dim=c(d,d,Ntotal))     ##
    param__Sigma_inv <- array(NA, dim=c(d,d,Ntotal)) ## innovation covariance matrix - Yixuan
  }
  ...
  noise <- get_noise(data, theta[,1])  # noise = data - signal
  ...
  if (corrected) {
    if (toggle) {
      ...
      phi.fit <- list(ar=param__phi[,,1],
                      ...
                      beta=param__beta[,1],  ## 
                      mu_beta=beta_prior,       ## include stuff for prior computation, iff toggle
      ...
      ## Parse f_param_half to likelihood to save redundant computations
      ##
      f_param <- psd_varma(lambda, phi.fit, sigma=NULL)
      f_param_half <- chol_cube(f_param$psd, excludeBoundary=F)
