    ...
    if (corrected && toggle) {
      ...
      for (jj in 1:var.order) {
        ...
        for (tt in 1:(n-var.order)) { ##
          ...
          V_beta_post_inv <- V_beta_post_inv + t(Z_t) %*% solve(sigma.fit) %*% Z_t ##
          param__beta_old <- param__beta_old + t(Z_t) %*% solve(sigma.fit) %*% Y_t ##  mimic var - Yixuan
        }
        V_beta_post <- solve(V_beta_post_inv)
        beta_prior.star <- V_beta_post %*% param__beta_old
        param__beta.star <- param__beta.old
        param__beta.star[indices_jj] <- MASS::mvrnorm(1, mu=beta_prior.star, Sigma=V_beta_post)[indices_jj]
